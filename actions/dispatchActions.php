...
function create_citation()
{
   ...
   $link = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
    ...
    //Fixed 10/06/19: Fixed the redirect. Will now redirect you to the correct department instead of mdt.php.

    $previous_location = $_SESSION['previous_location'];


    if($previous_location == "mdtState"){
        header("Location:".BASE_URL."/mdt.php?dep=state");
        }
    elseif($previous_location == "mdtSheriff"){
        header("Location:".BASE_URL."/mdt.php?dep=sheriff");
    }
    elseif($previous_location == "mdtHighway"){
        header("Location:".BASE_URL."/mdt.php?dep=highway");
    }
    elseif($previous_location == "mdtPolice"){
        header("Location:".BASE_URL."/mdt.php?dep=police");
    }
    elseif($previous_location == "mdtFire"){
        header("Location:".BASE_URL."/mdt.php?dep=fire");
    }
    elseif($previous_location == "mdtEMS"){
        header("Location:".BASE_URL."/mdt.php?dep=EMS");
    }
    elseif($previous_location == "mdtRoad"){
